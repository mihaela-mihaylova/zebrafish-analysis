...
for (c1_file in c1_list_of_files) {
 ...
 # print(c1_file)
  ...
  if (grepl("MIX", c1_file, ignore.case=TRUE)) { 
    mix_02h <- append(mix_02h, ifelse(is_empty(plt_02h),0,mean(plt_02h$int_length)))
    mix_24h <- append(mix_24h, ifelse(is_empty(plt_24h),0,mean(plt_24h$int_length)))     
    mix_46h <- append(mix_46h, ifelse(is_empty(plt_46h),0,mean(plt_46h$int_length)))
    mix_68h <- append(mix_68h, ifelse(is_empty(plt_68h),0,mean(plt_68h$int_length)))
  } else if (grepl("NT", c1_file, ignore.case=TRUE)) {
    nt_02h <- append(nt_02h, ifelse(is_empty(plt_02h),0,mean(plt_02h$int_length)))
    nt_24h <- append(nt_24h, ifelse(is_empty(plt_24h),0,mean(plt_24h$int_length)))
    nt_46h <- append(nt_46h, ifelse(is_empty(plt_46h),0,mean(plt_46h$int_length)))
    nt_68h <- append(nt_68h, ifelse(is_empty(plt_68h),0,mean(plt_68h$int_length)))
  } else if (grepl("RESCUE", c1_file, ignore.case=TRUE)) {
    rescue_02h <- append(rescue_02h, ifelse(is_empty(plt_02h),0,mean(plt_02h$int_length)))
    rescue_24h <- append(rescue_24h, ifelse(is_empty(plt_24h),0,mean(plt_24h$int_length)))
    rescue_46h <- append(rescue_46h, ifelse(is_empty(plt_46h),0,mean(plt_46h$int_length)))
    rescue_68h <- append(rescue_68h, ifelse(is_empty(plt_68h),0,mean(plt_68h$int_length)))
  }
}
...
plt_all_02h <- ggplot(all_02h, aes(x = factor(type), y = mean_int_len, colour=factor(type))) +
  geom_beeswarm()+
  theme_classic()+
  labs(x="Type",y="Mean interaction length per movie", title="0-2h")+
  scale_y_continuous(limits = c(0,max_all_int), breaks = seq(0,max_all_int, by=0.5))+
  theme(legend.position="none")

plt_all_24h <- ggplot(all_24h, aes(x = factor(type), y = mean_int_len, colour=factor(type))) +
  geom_beeswarm()+
  theme_classic()+
  labs(x="Type",y="", title="2-4h")+
  scale_y_continuous(limits = c(0,max_all_int), breaks = seq(0,max_all_int, by=0.5))+
  theme(legend.position="none")

plt_all_46h <- ggplot(all_46h, aes(x = factor(type), y = mean_int_len, colour=factor(type))) +
  geom_beeswarm()+
  theme_classic()+
  labs(x="Type",y="", title="4-6h")+
  scale_y_continuous(limits = c(0,max_all_int), breaks = seq(0,max_all_int, by=0.5))+
  theme(legend.position="none")

plt_all_68h <- ggplot(all_68h, aes(x = factor(type), y = mean_int_len, colour=factor(type))) +
  geom_beeswarm()+
  theme_classic()+
  labs(x="Type",y="", title="6-8h")+
  scale_y_continuous(limits = c(0,max_all_int), breaks = seq(0,max_all_int, by=0.5))+
...
name <-paste0("plots/mean_inter_length_per_movie_per_type.pdf")
pdf(name, width=10, height=3, pointsize = 9, useDingbats=FALSE) #units="in",res=600)
